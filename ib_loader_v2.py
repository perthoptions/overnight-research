...
def fetch(args):
    ...
    print(f"history  : {args.months} months (~{int(args.months * 21)} sessions) "
          f"-> ~{args.months} chunks each")
    ...
    # A cached file from a shorter run does not satisfy a longer request.
    # Skip only when the cache actually spans the window being asked for.
    required_start = (pd.Timestamp.now(tz=NY).normalize()
                      - pd.DateOffset(months=args.months)
                      + pd.Timedelta(days=25))     # tolerance for listing date / holidays

    for i, sym in enumerate(tickers, 1):
        path = CACHE / f"{sym}.parquet"
        if path.exists() and not args.force:
            try:
                have = pd.read_parquet(path, columns=["date"])
                have_start = pd.to_datetime(have["date"]).min()
                n_sess = pd.to_datetime(have["date"]).dt.normalize().nunique()
                if have_start <= required_start:
                    skipped += 1
                    continue
                print(f"[{i:>4}/{len(tickers)}] {sym:<6} cache starts "
                      f"{have_start.date()} ({n_sess} sessions) -- too short, re-fetching")
            except Exception as e:
                print(f"[{i:>4}/{len(tickers)}] {sym:<6} unreadable cache "
...
def main():
    ...
    f.add_argument("--months", type=int, default=24)
